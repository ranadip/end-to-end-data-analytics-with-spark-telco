...
def fnSparkMlopsPipeline(
    ...
    ).after(createManagedDatasetStep).set_display_name("Model training")
    ...
    with Condition(
        ...
        modelUploadStep = ModelUploadOp(
                project=project_id,
                location=location,
                display_name="pyspark-customer-churn-RF",
                unmanaged_container_model=importModelArtifactStep.outputs["model"],
            ).after(importModelArtifactStep)
        endpointStep = EndpointCreateOp(
                project=project_id,
                location=location,
                display_name="pyspark-customer-churn-RF"
            ).after(modelUploadStep)
        _ = ModelDeployOp(
...
# {{ Start main
    ...
    try:
        ...
        pipeline = vertex_ai.PipelineJob(
            ...
            pipeline_root='gs://s8s_model_bucket-{}/customer-churn-model/pipelines'.format(projectNbr),
            parameter_values={
                'project_id': projectID,
                'pipeline_id': pipelineID,
                'location': gcpRegion,
                'service_account': 's8s-lab-sa@spark-hackfest-dev.iam.gserviceaccount.com',
                'subnetwork_uri': 'projects/{}/regions/{}/subnetworks/spark-snet'.format(projectID,gcpRegion),
                'spark_phs_nm': 'projects/{}/regions/{}/clusters/s8s-sphs-{}'.format(projectID,gcpRegion,projectNbr),
                ...
                'model_training_main_py_fqn': 'gs://s8s_code_bucket-{}/modelbuilding/model_building.py'.format(projectNbr),
        ...
        umsa_fqn="s8s-lab-sa@spark-hackfest-dev.iam.gserviceaccount.com"
        pipeline.submit(service_account=umsa_fqn, network="projects/{}/global/networks/s8s-vpc-{}".format(projectNbr,projectNbr))
        pipeline.wait()
