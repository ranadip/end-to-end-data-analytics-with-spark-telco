...
resource "null_resource" "data_science_notebook_customization" {
    provisioner "local-exec" {
        command = "cp ${path.module}/scripts-templates/lab02-model_training.ipynb ${path.module}/scripts-hydrated/ && sed -i s/YOUR_PROJECT_NBR/${local.project_id}/g ${path.module}/scripts-hydrated/lab02-model_training.ipynb && sed -i s/YOUR_PROJECT_ID/${local.project_id}/g ${path.module}/scripts-hydrated/lab02-model_training.ipynb"
        interpreter = ["bash", "-c"]
    }
}
